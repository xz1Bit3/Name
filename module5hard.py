...
class UrTube:
    ...
    def log_in(self, login: str, password: str) -> None: #Метод log_in, который принимает на вход аргументы

        for user in self.users:
            if login == user.nickname and password == user.password:
                self.current_user = user

    def register(self, nickname: str, password: str, age: int) -> None: # Метод register, который принимает три аргумента: nickname, password, age
...
if __name__ == '__main__':
    ...
    v2 = Video('Для чего девушкам парень программист?', 10, adult_mode=False)
